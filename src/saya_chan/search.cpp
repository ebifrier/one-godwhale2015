...
namespace {
    ...
    inline Value razor_margin(Depth d) { return Value(0x200 + 0x25/*0x10*/ * int(d)); }
    ...
    const Value FutilityMarginQS = Value(0x100/*0x80*/);
...
bool think(Position& pos, const SearchLimits& limits, Move searchMoves[]) {
    ...
    if (Limits.maxNodes)
        ...
        NodesBetweenPolls = 10000; // polling間隔が長すぎると時間切れになる。
...
// 手番のときに王手をかけている状態は本来ありえない(前の手で王手回避していないか、自殺手を指していることになる)
        ...
        tte = TT.probe(posKey, pos.hand_value_of_side());
#else
...
split_point_start: // At split points actual search starts from here
               ...
               && !thread.cutoff_occurred())
        {
                ...
                && ext < ONE_PLY)
            {
                ...
                {
					// ToDo: depth引くのでいいの？これもう少し何らかの調整が要るのでは…。
					// StockfishはONE_PLY == 2の実装では、残り深さ6(depth = 12)においてこのマージン値は12。
					// この値のままだと、将棋だとほとんどがsingular扱いになってしまう？ということで適当に変えてみる
            ...
            if (value > bestValue)
            {
                ...
                /*if (RootNode && value > alpha) {
                    godwhale::client::update_value(
                        move, value, alpha, beta, pos.nodes_searched(), Rml[0].pv);
                }*/
#endif
                bestValue = value;
                ss->bestMove = move;
...
#if defined(NANOHA)
            TT.store(posKey, pos.hand_value_of_side(), value_to_tt(bestValue, ss->ply), vt, depth, move, ss->eval, ss->evalMargin);
#else
    ...
    Value qsearch(Position& pos, SearchStack* ss, Value alpha, Value beta, Depth depth) {
...
#if defined(NANOHA)
        tte = TT.probe(pos.get_key(), pos.hand_value_of_side());
#else
        ...
        else
        {
            ...
            if (bestValue >= beta)
            {
                if (!tte)
#if defined(NANOHA)
                    TT.store(pos.get_key(), pos.hand_value_of_side(), value_to_tt(bestValue, ss->ply), VALUE_TYPE_LOWER, DEPTH_NONE, MOVE_NONE, ss->eval, evalMargin);
#else
...
#if defined(NANOHA)
        TT.store(pos.get_key(), pos.hand_value_of_side(), value_to_tt(bestValue, ss->ply), vt, ttDepth, move, ss->eval, evalMargin);
#else
...
#if defined(NANOHA)
        int dummy = 0;
        while (   (tte = TT.probe(pos.get_key(), pos.hand_value_of_side())) != NULL
#else
    ...
    void RootMove::insert_pv_in_tt(Position& pos) {
        ...
        do {
            k = pos.get_key();
#if defined(NANOHA)
            tte = TT.probe(k, pos.hand_value_of_side());
#else
            tte = TT.probe(k);
#endif
